...
def receive_stock_history(request):
    ...
    if main_obj.exists():
        for i in range(0,main_obj.count()):
            data=[ApprovedUsers.objects.filter(id_no=main_obj.values('distributor_id')[i]['distributor_id']).values('name')[0]['name'],main_obj.values('product_id')[i]['product_id'],SetProduct.objects.filter(Product_id=main_obj.values('product_id')[i]['product_id']).values('name')[0]['name'],main_obj.values('product_quantity')[i]['product_quantity'],main_obj.values('date')[i]['date']]
            prod_data.append(data)
